...
describe('ProjectIndexManager', () => {
  ...
  describe('constructor', () => {
    it('should create manager with custom project root', () => {
      ...
      expect(manager.getAxMdPath()).toBe(path.join(testRoot, 'AX.md'));
    });
    ...
    it('should create manager with default project root (cwd)', () => {
      ...
      expect(manager.getAxMdPath()).toContain('AX.md');
    });
  });

  describe('AX.md support', () => {
    it('should return correct AX.md path', () => {
      const testRoot = path.join(path.sep, 'project');
      const manager = new ProjectIndexManager(testRoot);
      expect(manager.getAxMdPath()).toBe(path.join(testRoot, 'AX.md'));
    });

    it('should return true when AX.md exists', () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      const manager = new ProjectIndexManager('/project');
      expect(manager.axMdExists()).toBe(true);
    });

    it('should return false when AX.md does not exist', () => {
      vi.mocked(fs.existsSync).mockReturnValue(false);
      const manager = new ProjectIndexManager('/project');
      expect(manager.axMdExists()).toBe(false);
    });

    it('should load AX.md content', () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('# Project\n\nBuild commands here');
      const manager = new ProjectIndexManager('/project');
      expect(manager.loadAxMd()).toBe('# Project\n\nBuild commands here');
    });

    it('should return null when AX.md does not exist', () => {
      vi.mocked(fs.existsSync).mockReturnValue(false);
      const manager = new ProjectIndexManager('/project');
      expect(manager.loadAxMd()).toBe(null);
    });

    it('should cache AX.md content', () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('# Cached content');
      const manager = new ProjectIndexManager('/project');
  ...
  describe('getPromptContext', () => {
    it('should return null when no AX.md, summary, or index exists', () => {
      vi.mocked(fs.existsSync).mockReturnValue(false);
    ...
    it('should prioritize AX.md over JSON files', () => {
      // All files exist
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('AX.md')) {
          return '# My Project\n\n## Build Commands\n\npnpm build';
        }
        if (pathStr.includes('summary')) {
      ...
      // Should use AX.md content, not JSON
      expect(context).toContain('<project-context source="AX.md">');
      expect(context).toContain('# My Project');
      expect(context).toContain('pnpm build');
      expect(context).not.toContain('from-summary');
    });

    it('should strip HTML metadata comments from AX.md', () => {
      vi.mocked(fs.existsSync).mockImplementation((path) =>
        String(path).includes('AX.md')
      );
      vi.mocked(fs.readFileSync).mockReturnValue(
      ...
      expect(context).not.toContain('Generated by');
      expect(context).not.toContain('Last updated');
