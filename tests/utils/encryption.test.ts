...
describe('REQ-SEC-003: API Key Encryption', () => {
  ...
  describe('Encryption Security', () => {
    ...
    it('should fail to decrypt with invalid version', () => {
      const plaintext = 'my-secret-key';
      const encrypted = encrypt(plaintext);
      ...
      expect(() => decrypt(invalidVersion)).toThrow(/Unsupported encryption version/);
    });
  ...
  describe('Error Messages', () => {
    it('should provide helpful error for authentication failure', () => {
      const plaintext = 'my-secret-key';
      const encrypted = encrypt(plaintext);
      ...
      expect(() => decrypt(tampered)).toThrow(/Failed to decrypt API key/);
      expect(() => decrypt(tampered)).toThrow(/different machine/);
    });

    it('should provide helpful error for version mismatch', () => {
      const encrypted = encrypt('test');
      encrypted.version = 999;

      expect(() => decrypt(encrypted)).toThrow(/Unsupported encryption version: 999/);
    });
  });
});
