 ...
 *
 * This function:
 * 1. Loads configs from .ax-cli/settings.json
 * 2. Loads configs from .automatosx/config.json (if exists)
 * 3. Auto-migrates legacy formats
 * 4. Merges both sources (ax-cli takes precedence)
 * 5. Validates final configs
 */
export function loadMCPConfig(): MCPConfig {
  ...
  // Load ax-cli servers
  const axCliRawServers = projectSettings.mcpServers
    ? Object.values(projectSettings.mcpServers)
    : [];

  // Process ax-cli servers with migration
  const axCliServers: MCPServerConfig[] = [];
  let axCliHadLegacy = false;

  for (const server of axCliRawServers) {
    // Detect format
    ...
    if (detection.isLegacy) {
      ...
      if (result.success && result.data) {
        ...
        if (replWarning) {
          const serverName = (result.data as MCPServerConfig).name;
          if (!shownReplWarnings.has(serverName)) {
            shownReplWarnings.add(serverName);
            console.warn(formatWarning('Skipping problematic MCP server:', [replWarning]));
          }
          continue; // Skip this server instead of causing timeout
        }
  ...
  if (axCliServers.length > 0 || mergedServers.length > 0) {
    ...
    if (!hasShownMigrationWarnings && (mergeResult.conflicts.length > 0 || process.env.DEBUG)) {
      if (process.env.DEBUG) {
        console.log(formatMergeResult(mergeResult));
      }

      const recommendation = getConfigRecommendation(mergeResult);
      if (recommendation && mergeResult.conflicts.length > 0) {
