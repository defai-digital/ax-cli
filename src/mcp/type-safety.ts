...
export class MutexToken {
  // @ts-expect-error - Brand field is intentionally unused (for type safety only)
...
// Example 1: Branded types prevent mixing
function connectToServer(name: ServerName): void {
  console.log(`Connecting to ${name}`);
}

// This won't compile:
// connectToServer("github"); // Error: string is not ServerName

// This will compile:
const serverName = createServerName("github");
if (serverName) {
  connectToServer(serverName); // ✓ Safe
}

// Example 2: State machine prevents invalid transitions
let state: ConnectionState = { status: 'idle', serverName: 'github' as ServerName };
...
// state = transition(state, { status: 'disconnecting', ... }); // Error!

// Example 3: Non-empty array
const servers: ServerName[] = [];
// getFirstServer(servers); // Won't compile - might be empty

const nonEmpty = asNonEmpty(servers);
if (nonEmpty) {
  getFirstServer(nonEmpty); // ✓ Safe
}

// Example 4: Result type forces error handling - see integration tests for real usage
