  ...
  private async detectProjectName(): Promise<string> {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        this.warnings.push('Failed to parse package.json');
      }
    }
  ...
  private async detectTechStack(): Promise<string[]> {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        this.warnings.push('Failed to parse package.json for tech stack');
      }
    }
  ...
  private async detectConventions(): Promise<CodeConventions> {
    ...
    if (fs.existsSync(tsconfigPath)) {
      try {
        ...
        this.warnings.push('Failed to parse tsconfig.json');
      }
    }
  ...
  private async detectScripts(): Promise<ProjectInfo['scripts']> {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        this.warnings.push('Failed to parse scripts from package.json');
      }
    }
