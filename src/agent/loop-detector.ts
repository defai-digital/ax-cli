  ...
  private getSuggestion(toolName: string, _args: Record<string, unknown>): string {
    switch (toolName) {
      case 'str_replace_editor':
        return 'The text to replace may not match exactly. I should verify the file contents and adjust the search string.';
      case 'bash':
      case 'execute_bash':
        return 'The command may need adjustment. I should check the error output and try a different approach.';
      case 'view_file':
      case 'read_file':
        return 'I may already have the information I need from previous reads.';
      case 'search':
      case 'search_files':
        return 'I should try a different search query or look in a different location.';
      default:
        return 'I should try a different approach.';
    }
  }
