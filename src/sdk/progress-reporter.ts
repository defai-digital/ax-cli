...
export class ProgressReporter extends EventEmitter {
  ...
  report(event: Omit<ProgressEvent, 'timestamp'>): void {
    ...
    // Without this, external code could modify the metadata (including nested objects)
    // after event emission and corrupt the event data that listeners received
    // Using structuredClone for proper deep cloning of nested objects
    const timestamp = Date.now();
    const clonedMetadata = event.metadata ? structuredClone(event.metadata) : undefined;
    ...
    // (or vice versa). Each emission gets its own deep copy of metadata.
    this.emit('progress', {
      ...event,
      metadata: clonedMetadata ? structuredClone(clonedMetadata) : undefined,
      timestamp,
    });
