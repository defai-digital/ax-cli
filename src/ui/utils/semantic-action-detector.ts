...
export function detectSingleAction(entry: ChatEntry): SemanticAction {
  ...
  if (toolName === 'view_file' || toolName === 'read_file') {
    ...
    if (CONFIG_FILE_PATTERNS.test(filePath)) {
  ...
  // Edit operations
  // BUG FIX: Added multi_edit tool support
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    const actualTool = parts.length >= 3 ? parts.slice(2).join('__').toLowerCase() : '';

    // Infer action from MCP tool name patterns
    if (actualTool.includes('read') || actualTool.includes('get') || actualTool.includes('list') || actualTool.includes('fetch')) {
      return SemanticAction.Exploring;
    }
    if (actualTool.includes('write') || actualTool.includes('edit') || actualTool.includes('update') || actualTool.includes('create')) {
      return SemanticAction.Implementing;
    }
    if (actualTool.includes('search') || actualTool.includes('find') || actualTool.includes('query')) {
      return SemanticAction.Searching;
    }
    if (actualTool.includes('test') || actualTool.includes('check') || actualTool.includes('validate')) {
      return SemanticAction.Testing;
    }
    if (actualTool.includes('build') || actualTool.includes('compile')) {
      return SemanticAction.Building;
    }
