...
function detectChangePatterns(content: string): string[] {
  ...
  if (lower.includes('+interface') || /\+\s*type\b/.test(lower) || lower.includes('+class ')) {
    patterns.push('added types');
  } else if (lower.includes('interface') || /\btype\s+\w/.test(lower)) {
  ...
  if (/\bapi\b/.test(lower) || lower.includes('endpoint') || lower.includes('router') || /\bhandler\b/.test(lower)) {
    patterns.push('modified API');
  }

  // Database/models - BUG FIX: Use word boundary regex to avoid false positives like "remodel"
  if (lower.includes('schema') || /\bmodel\b/.test(lower) || lower.includes('migration') || lower.includes('database')) {
  ...
  if (lower.includes('component') || /\brender\b/.test(lower) || lower.includes('jsx') || lower.includes('tsx')) {
...
export function getBriefToolSummary(content: string, toolName: string): string {
  ...
  switch (toolName) {
    ...
    case 'multi_edit':
      ...
      const countMatch = content.match(/(\d+)\s*edit/i);
      if (countMatch) {
      ...
      const editMatches = content.match(/edit|applied|replaced|updated/gi) || [];
