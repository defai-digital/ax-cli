...
/**
 * Track string literal state while parsing text
 * Returns whether we're inside any string literal after processing
 */
...
/**
 * Parse text and return final string state (for checking if line ends in string)
 * Uses escape-aware quote tracking
 * BUG FIX: Use flag-based escape tracking instead of broken prevChar logic
 */
function getStringState(text: string): StringState {
  ...
  for (const char of text) {
    if (isEscaped) {
      // Previous char was backslash, skip this character
      isEscaped = false;
      continue;
    }

    if (char === '\\') {
      // This is an escape character, mark next char as escaped
      isEscaped = true;
      continue;
    }

    // Not escaped, check for quote characters
...
/**
 * Check if a character or sequence appears outside of strings in text
 * BUG FIX: Use consistent flag-based escape tracking
 */
function hasCharOutsideStrings(text: string, chars: string[]): boolean {
  ...
  for (const c of text) {
    ...
    // Check for target characters outside of strings
    if (!inSingleQuote && !inDoubleQuote && !inBacktick && chars.includes(c)) {
      return true;
    }
  }
  return false;
...
function isIncompleteInput(
  ...
  if (smartDetection.checkBrackets) {
    ...
    for (const char of trimmed) {
      ...
      // Track string state (not escaped)
      if (char === "'" && !inDoubleQuote && !inBacktick) inSingleQuote = !inSingleQuote;
      else if (char === '"' && !inSingleQuote && !inBacktick) inDoubleQuote = !inDoubleQuote;
      else if (char === '`' && !inSingleQuote && !inDoubleQuote) inBacktick = !inBacktick;
  ...
  if (smartDetection.checkOperators) {
    ...
    const state = getStringState(lastLine);
  ...
  if (smartDetection.checkStatements) {
    ...
    if (!isInsideString(state)) {
      ...
      if (incompleteKeywords.includes(lastWord)) return true;
