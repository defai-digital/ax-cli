...
// BUG FIX: Use synchronous initialization flag to prevent race condition
let mcpManager: MCPManager | null = null;
let mcpManagerInitializing = false;

/**
 * Get or create the MCP manager singleton
 * BUG FIX: Added initializing flag to prevent duplicate instance creation
 * during concurrent access
 */
export function getMCPManager(): MCPManager {
  if (!mcpManager) {
    // Double-check with initializing flag to prevent race condition
    if (mcpManagerInitializing) {
      // Another call is initializing, spin-wait (rare in practice)
      // This is acceptable since MCPManager construction is synchronous and fast
      while (mcpManagerInitializing && !mcpManager) {
        // Busy wait - this should be extremely rare and brief
      }
      if (mcpManager) return mcpManager;
    }
    mcpManagerInitializing = true;
    try {
      mcpManager = new MCPManager();
    } finally {
      mcpManagerInitializing = false;
    }
  }
  return mcpManager;
}
