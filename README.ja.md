...
> **重要：** Z.AI は公式 CLI ツール **OpenCode** をリリースしました。GLM/Z.AI ユーザーには ax-glm の代わりに OpenCode を直接ご利用いただくことをお勧めします。OpenCode を始める：https://opencode.ai。ax-glm クラウドパッケージは非推奨となり、公式 Z.AI ソリューションを優先してこのリポジトリから削除されました。
>
> **注意：** ローカル GLM モデル（GLM-4.6、CodeGeeX4）は引き続き `ax-cli` を通じて完全にサポートされており、Ollama、LMStudio、または vLLM を使用したオフライン推論が可能です。下記の[ローカル/オフラインモデル](#ローカルオフラインモデル-ax-cli)セクションをご覧ください。
...
> **GLM/Z.AI ユーザー：** ax-glm の代わりに Z.AI 公式の [OpenCode CLI](https://opencode.ai) をご利用ください。
...
| 機能 | 説明 |
|------|------|
| **プロバイダー最適化** | Grok (xAI) のファーストクラスサポート、プロバイダー固有のパラメータ付き |
...
- ax-grok は完全なツールチェーン（ファイル編集、MCP、bash）とプロジェクトメモリ機能を提供します。

> **GLM/Z.AI ユーザー：** Z.AI 公式の [OpenCode CLI](https://opencode.ai) をご利用ください。
...
```bash
ax-grok              # インタラクティブCLIセッションを開始
ax-grok --continue   # 前回の会話を再開
ax-grok -c           # 短縮形
```
...
```bash
ax-grok -p "このコードベースを分析"
ax-grok -p "TypeScriptエラーを修正" -d /path/to/project
```
...
| `.ax-grok/settings.json` | プロジェクトオーバーライド |
| `.ax-grok/CUSTOM.md` | カスタムAI指示 |
| `ax.index.json` | 共有プロジェクトインデックス（ルートにあり） |
...
インタラクティブモード（`ax-grok`）で、自然に質問するだけ：
...
AX CLI は共有コアに基づいてモジュラーアーキテクチャを使用：
...
> **GLM/Z.AI ユーザー：** ax-glm は非推奨です。Z.AI 公式の [OpenCode CLI](https://opencode.ai) をご利用ください。
