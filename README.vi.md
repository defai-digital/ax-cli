...
> 📖 Bản dịch này dựa trên [README.md @ v5.2.0](./README.md)
...
| Lệnh | Mô tả |
|------|------|
| `/init` | Tạo ngữ cảnh dự án AX.md (xem [Khởi tạo dự án](#khởi-tạo-dự-án)) |
...
Lệnh `/init` tạo file `AX.md` ở thư mục gốc dự án — file ngữ cảnh tổng hợp giúp AI hiểu codebase.

### Cách dùng cơ bản

```bash
ax-grok
> /init                    # Phân tích chuẩn (khuyến nghị)
> /init --depth=basic      # Quét nhanh cho dự án nhỏ
> /init --depth=full       # Phân tích sâu kèm bản đồ kiến trúc
> /init --depth=security   # Bao gồm audit bảo mật (secrets, API nguy hiểm)
```

### Mức độ phân tích

| Mức | Phân tích | Phù hợp |
|-----|-----------|---------|
| `basic` | Tên, ngôn ngữ, tech stack, scripts | Thiết lập nhanh, dự án nhỏ |
| `standard` | + Thống kê code, phân tích test, tài liệu | Hầu hết dự án (mặc định) |
| `full` | + Kiến trúc, phụ thuộc, hotspots, hướng dẫn | Codebase lớn |
| `security` | + Quét secrets, phát hiện API nguy hiểm, mẫu auth | Dự án nhạy cảm bảo mật |

### Đầu ra thích ứng

Lệnh `/init` tự điều chỉnh độ chi tiết theo độ phức tạp của dự án:

| Quy mô dự án | Số file | Đầu ra điển hình |
|------------|--------|------------------|
| Nhỏ | <50 file | Ngắn gọn, chỉ thông tin thiết yếu |
| Vừa | 50-200 file | Tài liệu chuẩn |
| Lớn | 200-500 file | Chi tiết kèm ghi chú kiến trúc |
| Enterprise | 500+ file | Toàn diện với mọi mục |

### Tuỳ chọn

| Tuỳ chọn | Mô tả |
|---------|------|
| `--depth=<level>` | Thiết lập độ sâu (basic, standard, full, security) |
| `--refresh` | Cập nhật AX.md hiện có với phân tích mới |
| `--force` | Tạo lại dù AX.md đã tồn tại |

### File được tạo

| File | Mục đích |
|------|---------|
| `AX.md` | File ngữ cảnh AI chính (luôn tạo) |
| `.ax/analysis.json` | Dữ liệu phân tích sâu (chỉ full/security) |

### Cơ chế injection ngữ cảnh

Khi bắt đầu cuộc trò chuyện, AX CLI tự đọc `AX.md` và đưa vào cửa sổ ngữ cảnh của AI. Điều này có nghĩa:

1. **AI hiểu dự án của bạn** - lệnh build, tech stack, quy ước
2. **Không cần giải thích lại** - AI nhớ cấu trúc dự án
3. **Gợi ý code tốt hơn** - theo đúng pattern và quy tắc hiện có
...
**Thứ tự ưu tiên** (khi có nhiều file ngữ cảnh):
1. `AX.md` (khuyến nghị) - Định dạng file đơn mới
2. `ax.summary.json` (legacy) - Tóm tắt JSON
3. `ax.index.json` (legacy) - Chỉ mục JSON đầy đủ

### Di chuyển từ định dạng legacy

Nếu bạn có file legacy (`.ax-grok/CUSTOM.md`, `ax.index.json`, `ax.summary.json`), hãy chạy:

```bash
> /init --force
```

Lệnh này sẽ tạo định dạng file đơn mới `AX.md`. Sau đó có thể xóa các file legacy.
...
| `AX.md` | File ngữ cảnh dự án (tạo bởi `/init`) |
