 ...
 *
 * Manages project context files for AI assistant context injection.
 *
 * Priority order:
 * 1. AX.md (primary) - Generated by /init command, markdown format
 * 2. ax.index.json + ax.summary.json (legacy) - JSON format for backward compatibility
 *
 * Handles:
 * - Loading and caching context files
 * - Checking staleness (24-hour threshold)
 * - Auto-regenerating when stale
 * - Providing project context for system prompts
 */
...
export class ProjectIndexManager {
  ...
  private cachedAxMd: string | null = null;
  ...
  constructor(projectRoot: string = process.cwd()) {
    ...
    this.axMdPath = path.join(projectRoot, FILE_NAMES.AX_MD);
  }

  /**
   * Get the path to AX.md
   */
  getAxMdPath(): string {
    return this.axMdPath;
  }

  /**
   * Check if AX.md exists (primary context file)
   */
  axMdExists(): boolean {
    return fs.existsSync(this.axMdPath);
  }

  /**
   * Load AX.md content
   */
  loadAxMd(): string | null {
    if (this.cachedAxMd) {
      return this.cachedAxMd;
    }
    ...
    try {
      const content = fs.readFileSync(this.axMdPath, 'utf-8');
      this.cachedAxMd = content;
      return content;
    } catch {
      return null;
    }
  ...
  /**
   * Check if AX.md is stale (older than 24 hours)
   */
  isAxMdStale(): boolean {
    if (!this.axMdExists()) {
      return true;
    ...
    try {
      const stats = fs.statSync(this.axMdPath);
  ...
  /**
   * Get AX.md age in hours
   */
  getAxMdAgeHours(): number | undefined {
    if (!this.axMdExists()) {
      return undefined;
    }
   ...
   * Returns null if no context files exist
   *
   * Priority order:
   * 1. AX.md (primary) - Full markdown content, wrapped in <project-context> tags
   * 2. ax.summary.json (legacy) - Pre-computed JSON summary
   * 3. ax.index.json (legacy) - Dynamic summary generation
   *
   * The AI can read AX.md or ax.index.json directly if it needs more details.
   */
  getPromptContext(): string | null {
    // PRIMARY: Try AX.md first (new format from /init)
    const axMdContent = this.loadAxMd();
    if (axMdContent) {
      return this.formatAxMdForPrompt(axMdContent);
    }

    // LEGACY FALLBACK 1: Try pre-computed summary
    const summaryData = this.loadSummaryData();
    if (summaryData) {
      return this.formatSummaryForPrompt(summaryData);
    }

    // LEGACY FALLBACK 2: Dynamically generate from index
    const content = this.load();
    if (!content) {
      return null;
    }
  ...
  /**
   * Format AX.md content for prompt injection
   * Wraps the markdown content in <project-context> tags for clear delineation
   *
   * @param content The raw AX.md content
   * @returns Formatted context for prompt injection
   */
  private formatAxMdForPrompt(content: string): string {
    // Strip the HTML comment metadata header (<!-- ... -->) from the content
    // as it's not useful for the AI and saves tokens
    const cleanedContent = content.replace(/^<!--[\s\S]*?-->\s*/m, '').trim();

    // Wrap in project-context tags for clear delineation
    return `<project-context source="AX.md">
${cleanedContent}
</project-context>`;
  }
