  ...
  sessionsCmd
    ...
    .action(async (options) => {
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        ConsoleMessenger.error('status_commands.error_listing_sessions', { error: message });
        process.exit(1);
      }
    });
  ...
  sessionsCmd
    ...
    .action(async (options) => {
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        ConsoleMessenger.error('status_commands.error_cleaning_sessions', { error: message });
        process.exit(1);
      }
    });
