...
function parseDiffWithLineNumbers(diffContent: string): DiffLine[] {
  ...
  for (const line of lines) {
    ...
    if (hunkMatch) {
      // BUG FIX: Guard against NaN from invalid parseInt (fallback to 0)
      currentOldLine = parseInt(hunkMatch[1], 10) || 0;
      currentNewLine = parseInt(hunkMatch[2], 10) || 0;
      inHunk = true;
    ...
    >
      {displayableLines.reduce<React.ReactNode[]>((acc, line, index) => {
        ...
        if (
          ...
          // BUG FIX: Ensure terminalWidth is positive to avoid empty string or negative repeat
          const safeWidth = Math.max(terminalWidth || 80, 10);
          acc.push(
            <Box key={`gap-${index}`}>
              <Text wrap="truncate">{'═'.repeat(safeWidth)}</Text>
            </Box>,
          );
        }
