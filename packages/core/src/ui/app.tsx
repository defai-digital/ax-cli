...
export default function App({ agent }: Props) {
  ...
  // Note: Using process.exit(0) instead of useApp().exit() because:
  // 1. useApp().exit() can leave terminal in raw mode on some terminals
  // 2. process.exit(0) ensures proper cleanup of stdin/stdout streams
  // 3. Consistent exit behavior across different terminal emulators
